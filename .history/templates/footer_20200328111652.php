<footer class="page-footer font-small blue pt-4 bg-light" style="color: black">

    <div class="container-fluid text-center text-md-left">

        <div class="row">

            <div class="col-md-6 mt-md-0 mt-3">

                <img src="assets/images/logo.jpg" alt="">
                <h5 class="text-uppercase">Limegrove Cinemas</h5>
                <p style="height: 100px">Some information about the company and their sponsors will go here.</p>

            </div>

            <hr class="clearfix w-100 d-md-none pb-3">

            <div class="col-md-3 mb-md-0 mb-3">

                <h5 class="text-uppercase bold">Latest Movies</h5>

                <ul class="list-unstyled">
                    <li>
                        <a href="#!">A New Sports Movie 1 <span style="float: right"><i class="fas fa-angle-right"></i></span></a>
                        <br>
                        <p><i class="far fa-clock"></i> Janurary 1, 2000</p>
                    </li>
                    <hr style="border-color: greenyellow;">
                    <li>
                        <a href="#!">Link 2</a>
                    </li>
                    <hr style="border-color: greenyellow;">
                    <li>
                        <a href="#!">Link 3</a>
                    </li>
                    <hr style="border-color: greenyellow;">
                    <li>
                        <a href="#!">Link 4</a>
                    </li>
                </ul>

            </div>
            <div class="col-md-3 mb-md-0 mb-3">

                <h5 class="text-uppercase bold">Blog</h5>

                <ul class="list-unstyled">
                    <li>
                        <a href="#!">Link 1</a>
                    </li>
                    <hr style="border-color: greenyellow;">
                    <li>
                        <a href="#!">Link 2</a>
                    </li>
                    <hr style="border-color: greenyellow;">
                    <li>
                        <a href="#!">Link 3</a>
                    </li>
                    <hr style="border-color: greenyellow;">
                    <li>
                        <a href="#!">Link 4</a>
                    </li>
                </ul>

            </div>

        </div>

    </div>

</footer>

</body>

</html>